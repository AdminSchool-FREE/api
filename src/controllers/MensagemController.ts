...
import {
  buscarNotificacoesReponsaveisAluno,
  inserirNotificacoes,
...
class MensagemController {
  async enviarMensagemResponsavelAluno(app: FastifyInstance) {
    ...
    app.post('/notificar/responsaveis', async (req, res) => {
      const { mensagem, idAluno, alunos } = await bodyMensagem.parseAsync(
        req.body,
      )
        ...
        if (
          ...
          if (idAluno && !alunos) {
            ...
            if (responsaveisAluno) {
                ...
                responsaveisAluno.responsavel.TelefoneResponsavel.forEach(
                  (contato) => {
                    const contatoResponsavel =
                      contato.ddd.trim() + contato.telefone.trim()
              ...
              res.status(400).send({
                message:
                  'Responsável não possui telefone para envio de whatsapp',
              })
            }
          } else {
            const mensagens: Array<NovaNotificacaoProps> = []
            ...
            const listaResponsaveis = await buscarResponsaveisAlunos(
              alunos?.map((alunoSelecionado) => alunoSelecionado.id) ?? [],
            )
